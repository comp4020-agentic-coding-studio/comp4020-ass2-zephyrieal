---
title: Make It Worse
description:
  The semester's first build — take a working interface and make it
  significantly worse, without breaking it
week: 5
date: 2027-03-22
teachers:
  - idris-fenn
spec:
  - your redesign still functions end to end — nothing is actually broken
  - you can name the specific friction you introduced (cognitive load, hidden
    information, ambiguity, unnecessary decisions, waiting, harder error
    recovery) for each change
  - you can state, in one sentence per change, what behaviour you expect it
    to produce
related:
  - assessments/worst-interface
---

## Before the session

Bring a normal, working interface — yours or a supplied starter — that you
are prepared to make worse over the next two weeks.

## In the session

The brief is deceptively simple: make it significantly worse without breaking
it. You cannot remove functionality; you can only introduce friction. The
session works through the menu of options — more cognitive load,
harder-to-find information, new ambiguity, extra unnecessary decisions, added
waiting, error recovery that punishes mistakes — and you start applying them
to your own interface.

## Afterwards

This is the semester's first making exercise: from "I recognise bad design"
to "I can deliberately produce it." Keep building through
[week 6](/sessions/06-friction-mapping/) —
[Assessment 2, the Worst Interface](/assessments/worst-interface/), is due at
the end of that week.
